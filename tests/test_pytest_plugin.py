...
def test_talika_fixture_parse_returns_schema_records(talika):
    users = talika.parse([["name"], ["Alice"]], schema=UserTable)
...
def test_talika_fixture_converts_records_with_parse_as(talika):
    @dataclass(frozen=True)
    class User:
        name: str

    users = talika.parse_as(
        [["name"], ["Alice"]],
        User,
        schema=UserTable,
    )

    assert users == [User(name="Alice")]
